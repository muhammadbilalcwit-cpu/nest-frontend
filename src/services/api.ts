...
import type { ApiResponse, User, Company, Department, ActivityLog, PaginatedData } from '@/types';
...
export const usersApi = {
  getAll: () => api.get<ApiResponse<User[]>>('/users/getAll'),

  getById: (id: number) => api.get<ApiResponse<User>>(`/users/getById/${id}`),

  getByEmail: (email: string) => api.get<ApiResponse<User>>(`/users/getByEmail/${email}`),

  create: (data: Partial<User> & { password: string; departmentId?: number; companyId?: number; roleSlug?: string }) =>
  ...
  updateProfile: (data: { firstname?: string; lastname?: string; password?: string; currentPassword?: string }) =>
    api.put<ApiResponse<User>>('/users/profile', data),

  delete: (id: number) => api.delete<ApiResponse<{ deleted: boolean }>>(`/users/delete/${id}`),
...
export const companiesApi = {
  ...
  getById: (id: number) => api.get<ApiResponse<Company>>(`/companies/getById/${id}`),
  ...
  delete: (id: number) => api.delete<ApiResponse<{ deleted: boolean }>>(`/companies/delete/${id}`),
};
...
export const departmentsApi = {
  ...
  getById: (id: number) => api.get<ApiResponse<Department>>(`/departments/getById/${id}`),

  getByCompany: (companyId: number) => api.get<ApiResponse<Department[]>>(`/departments/getByCompany/${companyId}`),
  ...
  delete: (id: number) => api.delete<ApiResponse<{ deleted: boolean }>>(`/departments/delete/${id}`),
};

// Roles endpoints
export const rolesApi = {
  getAll: () => api.get<ApiResponse<{ id: number; name: string; slug: string }[]>>('/roles/getAll'),
};

// Activity logs endpoints (for company_admin)
export const activityLogsApi = {
  getAll: (params?: { page?: number; limit?: number; method?: string; search?: string }) => {
  ...
  getByUserId: (userId: number) => api.get<ApiResponse<ActivityLog[]>>(`/activity-logs/user/${userId}`),
};
